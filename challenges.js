...
function formatWithPadding(num1, str, num2){
  ...
  if (num1.length >= num2) {
    ...
    return num1.toString().padStart(num2, str)
  }
}
...
// keep dividing by 2
// if number % 2 === 0, then not prime
// if it is not 0, keep dividing by 2 (num % 2)
// if you keep on doing and moduly === number then it is prime
...
function isPrime(n) {
  if (Number.isInteger(n) && n > 1) {
    ...
    return divisors.length > 2 ? false : true
  } else {
    return false
  }
}
...
// for each arg in arr1, check and see if there is a matching arg in arr 2, 
// and arg !in finalArr
// if so push to finalArr
// outerloop: [1, 'a', true, 1, 1]
// innerloop: [true, 1, 'b', 1]
// pushedArg = whatever wants to go in
// finalArr: [1, true, 1]
// MUSTARDARRAY = [b]
// check pushedArg against third loop
// if finalArr doesn't .includes pushedArg, then push
...
//for every iteration, put an open bracket into a stack
// in the same itteration ,if you have a closing bracket, check the top of the stack
// if top of the stack === the opening bracket of the type, remove the openening bracket from the stack, continue,
// if it does not match the opening, return false  
...
// 192 base 256 power 3
//156 base " power 2
// 99 base " power 1
// 15 base " power 0
// calculate result
// str.split (,)=[192, 156, 99, 15]
// powerArray = [3,2,1,0]
// loop through string
// let sum = 0
...
//replace the _ or - and following lowercase letter with uppercase of that letter
//replace _/- with regex [_-]\w     underscore/hyphen followed by any charachter
//second arg for replace is a function taht pulls that section of string
//goes the the 1st index of that string (the letter) 
//and capitalizes it
